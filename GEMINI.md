...
* `0P_PLUGINS`: Plugin adapters (caveman / code-review-graph / capsule-kit) with native fallbacks.
* `90_MEMORY`: Project memory (nexus.project.yaml), short logs, conflict-safe writes.
...
* `04_TYPE`: Typography, modular scale, fluid type (320–2560px bounded), readability.
* `05_MOTION`: View Transitions, choreography, reduced motion.
* `06_LAYOUT`: 8-tier breakpoints (320–2560px+), balanced composition, svh/dvh/cqw, structural grid.
* `07_COMPONENTS`: Component anatomy, state trees, cross-viewport reflow, loading/error states.
...
### OPERATOR (Native / Specialized Contexts)
* `91_PORTFOLIO`: Portfolio & case studies, personal brand, storytelling.
* `92_FREELANCE`: Proposals, scoping, pricing, contracts, handoffs.
* `93_MOBILE`: React Native / Expo native apps (use 09_BUILD for web).
